...
\subsection{Post-approach: Context-based Refinement}\noindent

In the Berger-Levrault case study, business rules are documented in structured documents organized into a title and several sections. Each section contains a table listing the business rules associated with that section. We exploit this additional information by considering the document name and the section name as contextual information associated with each business rule.

We hypothesize that business rules sharing the same context are also implemented in related parts of the source code. In particular, we assume that methods implementing business rules associated with the same context are likely to be grouped within the same class. Based on this assumption, we introduce an additional filtering step after the main approach, referred to as \textit{Context-Based Refinement}.

For each business rule, we use the name of the document and the name of its corresponding section to refine the final set of candidate methods produced by the approach. We retain only the methods belonging to classes whose names contain either the document name or the section name associated with the business rule. The remaining methods are considered to be outside the relevant context and are therefore removed from the candidate set. This refinement step further reduces the search space by exploiting contextual information available in the business rule documentation.